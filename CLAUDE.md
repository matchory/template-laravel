...
- `composer.json`'s `fmt` script points Pint at the package's config with `--config`, because Pint
  ...
  `pint.json`.
- `phpstan.neon` includes the package's rulesets under `includes:`. The package also ships a
  `base.neon` of settings safe for any consumer, pulled in via the package's own auto-discovered
...
command, a queued job) belongs in a Domain Action: a single-purpose `{Action}Action` class with a
`handle()` method, living at `app/Domain/{Feature}/{Action}Action.php`. Entry points stay thin:
resolve the action via the container, call `handle()`, and return the result. Internal infrastructure
concerns that aren't themselves business logic, such as token refresh, third-party API clients, or
transport adapters, belong in `app/Support/` as plain service classes, not as Actions.
...
a default value, since a default causes Spatie to fall back to `null` for a missing field instead of
...
**Observers, not controllers.** Side effects that follow from a model changing, such as cache
invalidation, notifications, or syncing to a search index, belong in an observer registered on the
model, not inline in the controller or action that triggered the change.
