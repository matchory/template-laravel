...
package's Laravel, Pest, and complexity rulesets under `includes:`. The `phpstan/extension-installer`
plugin also auto-registers a `base.neon` for every installed PHPStan extension — that file must never
be included by hand, since the installer already wires it in.

**`cognitive_complexity` is declared locally, on purpose.** PHPStan resolves configuration by merging
every auto-discovered extension config, in install order, after the `includes:` list but before this
project's own `parameters:` block. Because the cognitive-complexity extension installs after
`matchory/coding-style`, its own defaults would silently win over whatever threshold the shared
package tries to set — so the threshold has to be restated locally to take effect. `phpstan.neon`
keeps `level`, `paths`, and `cognitive_complexity` local for this reason; everything else comes from
the shared package.
...
dependency — it checks that this repository actually consumes it the way it's meant to be consumed.
CI runs it on every push; run it locally after touching any of the files above.
...
no `database/` migrations or seeders, and no domain code — those are exactly the things a real
project adds first. See `CLAUDE.md` for the conventions to follow once you do.
